...
function axiosErrorHandler(err: any, req: Request, res: Response, next: NextFunction) {
  if (err instanceof AxiosError) {
    const status = err.response?.status ?? 501;
...
export { logErrors, errorHandler, boomErrorHandler, ormErrorHandler, logUndefinedRoutes, axiosErrorHandler };
